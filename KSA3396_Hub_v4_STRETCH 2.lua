...
local function OnClick()
    ...
    if hitPos then
        ...
        cube.Anchored   = false  -- unanchored أول شوية
        cube.CanCollide = true
        cube.CastShadow = true
        cube.CFrame     = CFrame.new(hitPos + Vector3.new(0, cubeSize / 2, 0))

        -- ===== حيلة FE: ألصق بشخصيتنا مؤقتاً (الكل يشوفه) =====
        if myHRPNow then
            cube.Parent = myHRPNow   -- داخل شخصيتنا = يرى الكل
            -- WeldConstraint يحافظ على موضع الكيوب
            local wc = Instance.new("WeldConstraint")
            wc.Part0   = cube
            wc.Part1   = myHRPNow
            wc.Parent  = cube
            -- بعد ثانية واحدة: انقله للـ Workspace وأنكره
            task.delay(1, function()
                if cube and cube.Parent then
                    wc:Destroy()
                    cube.Anchored = true
                    cube.Parent   = Workspace
                    pcall(function() cube:SetNetworkOwner(nil) end)
                end
            end)
        else
            cube.Anchored = true
...
local function DoCubeFling(target)
    ...
    -- ===== Visual cube (مرئي في الألعاب بدون FE) =====
    local cube = Instance.new("Part")
    ...
    cube.CanCollide   = true
    cube.CastShadow   = true
    cube.Position     = tHRP.Position + Vector3.new(0, 85, 0)
    -- ألصق المعكب بشخصيتنا مؤقتاً حتى يراه الكل (حيلة FE)
    cube.Parent = myHRP
    pcall(function()
        local w = Instance.new("WeldConstraint")
        w.Part0 = cube; w.Part1 = myHRP; w.Parent = cube
        Debris:AddItem(w, 0.5)
    end)
    task.delay(0.5, function() cube.Parent = Workspace end)

    -- Billboard label
    local bb  = Instance.new("BillboardGui", cube)
    bb.Size        = UDim2.new(0, 130, 0, 36)
    bb.StudsOffset = Vector3.new(0, 17, 0)
    bb.AlwaysOnTop = true
    local bbl = Instance.new("TextLabel", bb)
    bbl.Size                 = UDim2.new(1, 0, 1, 0)
    bbl.BackgroundTransparency = 1
    ...
    -- ===== Crash Fling — شخصيتنا كسلاح (يرى الكل حتى في FE) =====
    task.spawn(function()
        local savedCF = myHRP.CFrame
        -- اطر فوق الهدف
        myHRP.CFrame = CFrame.new(tHRP.Position + Vector3.new(0, 8, 0))
        task.wait(0.06)
        -- انفجر بسرعة هائلة (شخصيتنا مرئية للسيرفر)
        local crashBV = Instance.new("BodyVelocity", myHRP)
        crashBV.Velocity  = Vector3.new(math.random(-400,400), math.random(800,1100), math.random(-400,400))
        crashBV.MaxForce  = Vector3.new(1e9, 1e9, 1e9)
        Debris:AddItem(crashBV, 0.18)
        -- طريقة مباشرة أيضاً (للألعاب بدون FE)
        pcall(function()
            for _, v in ipairs(tHRP:GetChildren()) do
                if v:IsA("BodyVelocity") then v:Destroy() end
            end
            local fBV = Instance.new("BodyVelocity", tHRP)
            fBV.Velocity  = Vector3.new(math.random(-400,400), math.random(850,1100), math.random(-400,400))
    ...
    task.delay(0.4, function()
        ...
        Debris:AddItem(cube, 2.5)
    end)
end

MakeToggle("Fling القريبين تلقائياً (الكل يشوفه)", function(on)
    flingEnabled = on
    if on then
        local lastFling = 0
        flingLoop = RunService.Heartbeat:Connect(function()
            ...
            local myHRP  = myChar and myChar:FindFirstChild("HumanoidRootPart")
            if not myHRP then return end
            -- throttle: مرة كل 0.7 ثانية لتجنب الـ spam
            if tick() - lastFling < 0.7 then return end
            for _, p in ipairs(Players:GetPlayers()) do
                if p ~= LocalPlayer and p.Character then
                    local tHRP = p.Character:FindFirstChild("HumanoidRootPart")
                    if tHRP and (myHRP.Position - tHRP.Position).Magnitude < 35 then
                        lastFling = tick()
                        task.spawn(function()
                            local savedCF = myHRP.CFrame
                            -- Crash method (يرى الكل)
                            myHRP.CFrame = CFrame.new(tHRP.Position + Vector3.new(0, 4, 0))
                            task.wait(0.05)
                            local bv = Instance.new("BodyVelocity", myHRP)
                            bv.Velocity = Vector3.new(math.random(-350,350), math.random(700,950), math.random(-350,350))
                            bv.MaxForce = Vector3.new(1e9,1e9,1e9)
                            Debris:AddItem(bv, 0.15)
                            -- طريقة مباشرة أيضاً
                            pcall(function()
                                local bv2 = Instance.new("BodyVelocity", tHRP)
                                bv2.Velocity = Vector3.new(math.random(-300,300), math.random(600,900), math.random(-300,300))
...
MakeSection("📏 مط وتكبير الجسم (الكل يشوفه ✅)")

--[[
  يشتغل مع R6 وR15:
  - R15: يغير BodyScale Values داخل Humanoid (السيرفر يشوفه)
  - R6: يغير حجم الأجزاء مباشرة + يصلح Motor6D Joints (الكل يشوفه)
  الشخصية مملوكة للعميل = التغييرات تنتقل للسيرفر تلقائياً
]]

local isScaled       = false
local currentScaleX  = 1
local currentScaleY  = 1
local currentScaleZ  = 1
local scaleX_val     = 1]
local scaleY_val     = 1
local scaleZ_val     = 1
...
-- Motor6D الأصلية C0 لـ R6
local R6_JOINTS_C0 = {
    ["Root Joint"]      = CFrame.new(0, -1, 0) * CFrame.Angles(-math.pi/2, 0, math.pi),
    ["Neck"]            = CFrame.new(0, 1, 0) * CFrame.Angles(-math.pi/2, 0, math.pi),
    ["Left Shoulder"]   = CFrame.new(-1, 0.5, 0) * CFrame.Angles(0, -math.pi/2, 0),
    ["Right Shoulder"]  = CFrame.new(1, 0.5, 0) * CFrame.Angles(0, math.pi/2, 0),
    ["Left Hip"]        = CFrame.new(-1, -1, 0) * CFrame.Angles(0, -math.pi/2, 0),
    ["Right Hip"]       = CFrame.new(1, -1, 0) * CFrame.Angles(0, math.pi/2, 0),
}

-- حفظ الأجزاء الأصلية (R6)
local r6OriginalSizes = {}

local function IsR15(char)
    local hum = char and char:FindFirstChildOfClass("Humanoid")
    return hum and hum:FindFirstChild("BodyDepthScale") ~= nil
end

local function SaveR6Originals(char)
    r6OriginalSizes = {}
    for name, _ in pairs(R6_DEFAULT) do
        local p = char:FindFirstChild(name)
        if p and p:IsA("BasePart") then
            r6OriginalSizes[name] = p.Size
        end
    end
end

local function ApplyR6Scale(char, sx, sy, sz)
    -- غيّر حجم كل جزء
    for name, defSize in pairs(R6_DEFAULT) do
        local part = char:FindFirstChild(name)
        if part and part:IsA("BasePart") then
            part.Size = Vector3.new(defSize.X * sx, defSize.Y * sy, defSize.Z * sz)
        end
    end

    -- صلّح Motor6D Joints بعد التغيير في الحجم
    local torso = char:FindFirstChild("Torso")
    if torso then
        for _, motor in ipairs(torso:GetDescendants()) do
            if motor:IsA("Motor6D") then end
        end
        -- Root Joint
        local rootJoint = char:FindFirstChild("HumanoidRootPart") and
            char.HumanoidRootPart:FindFirstChild("RootJoint")
        if rootJoint then
            rootJoint.C0 = CFrame.new(0, -1 * sy, 0) * CFrame.Angles(-math.pi/2, 0, math.pi)
            rootJoint.C1 = CFrame.new(0, -1 * sy, 0) * CFrame.Angles(-math.pi/2, 0, math.pi)
        end
        -- Neck
        local neck = torso:FindFirstChild("Neck")
        if neck then
            neck.C0 = CFrame.new(0, 1 * sy, 0) * CFrame.Angles(-math.pi/2, 0, math.pi)
            neck.C1 = CFrame.new(0, -0.5 * sy, 0) * CFrame.Angles(-math.pi/2, 0, math.pi)
        end
        -- Left Shoulder
        local ls = torso:FindFirstChild("Left Shoulder")
        if ls then
            ls.C0 = CFrame.new(-1 * sx, 0.5 * sy, 0) * CFrame.Angles(0, -math.pi/2, 0)
            ls.C1 = CFrame.new(0.5 * sx, 0.5 * sy, 0) * CFrame.Angles(0, -math.pi/2, 0)
        end
        -- Right Shoulder
        local rs2 = torso:FindFirstChild("Right Shoulder")
        if rs2 then
            rs2.C0 = CFrame.new(1 * sx, 0.5 * sy, 0) * CFrame.Angles(0, math.pi/2, 0)
            rs2.C1 = CFrame.new(-0.5 * sx, 0.5 * sy, 0) * CFrame.Angles(0, math.pi/2, 0)
        end
        -- Left Hip
        local lh = torso:FindFirstChild("Left Hip")
        if lh then
            lh.C0 = CFrame.new(-1 * sx, -1 * sy, 0) * CFrame.Angles(0, -math.pi/2, 0)
            lh.C1 = CFrame.new(-0.5 * sx, 1 * sy, 0) * CFrame.Angles(0, -math.pi/2, 0)
        end
        -- Right Hip
        local rh = torso:FindFirstChild("Right Hip")
        if rh then
            rh.C0 = CFrame.new(1 * sx, -1 * sy, 0) * CFrame.Angles(0, math.pi/2, 0)
            rh.C1 = CFrame.new(0.5 * sx, 1 * sy, 0) * CFrame.Angles(0, math.pi/2, 0)
        end
    end
end

local function ApplyR15Scale(char, sx, sy, sz, headS)
    local hum = char:FindFirstChildOfClass("Humanoid")
    if not hum then return end
    local dv = hum:FindFirstChild("BodyDepthScale")
    local hv = hum:FindFirstChild("BodyHeightScale")
    local wv = hum:FindFirstChild("BodyWidthScale")
    local he = hum:FindFirstChild("HeadScale")
    if dv  then dv.Value  = sz end
    if hv  then hv.Value  = sy end
    if wv  then wv.Value  = sx end
    if he  then he.Value  = headS or math.max(sx, sy, sz) * 0.6 end
end

local function ApplyBodyScale(sx, sy, sz, headS)
    local char = LocalPlayer.Character
    if not char then return end

    if IsR15(char) then
        -- R15
        ApplyR15Scale(char, sx, sy, sz, headS)
    else
        -- R6
        if not next(r6OriginalSizes) then SaveR6Originals(char) end
        ApplyR6Scale(char, sx, sy, sz)
    end
...
local function ResetBodyScale()
    ...
    if IsR15(char) then
        ApplyR15Scale(char, 1, 1, 1, 1)
    else
        -- إعادة الأحجام الأصلية R6
        for name, origSize in pairs(r6OriginalSizes) do
            local part = char:FindFirstChild(name)
            if part and part:IsA("BasePart") then
                part.Size = origSize
            end
...
task.spawn(function()
    task.wait(0.8)
    local char = LocalPlayer.Character
    if char and not IsR15(char) then
        SaveR6Originals(char)
    end
end)

-- === سلايدرات ===
MakeSlider("↔️ عرض الجسم (X)", 1, 15, 1, function(v)
    scaleX_val = v
end)
MakeSlider("↕️ طول الجسم (Y)", 1, 15, 1, function(v)
    scaleY_val = v
end)
MakeSlider("↙️ عمق الجسم (Z)", 1, 15, 1, function(v)
    scaleZ_val = v
end)

MakeButton("✅ طبّق المط الآن (الكل يشوفه)", Color3.fromRGB(20, 110, 45), function()
    ApplyBodyScale(scaleX_val, scaleY_val, scaleZ_val)
end)

-- === بريسيتات جاهزة ===
MakeSection("📐 أشكال جاهزة (يشوفها الكل ✅)")

local stretchPresets = {
    -- name,                              sx,   sy,    sz,   head
    {"🏔️ ممطوط للطول (زي الصورة)",       1,    8,     1,    0.8 },
    {"🧱 عريض جداً",                      6,    1,     6,    1.5 },
    {"🔮 كيوب ضخم (مربع)",                4,    4,     4,    2.0 },
    {"🪲 مبسوط (عريض + قصير)",            5,    0.3,   5,    0.5 },
    {"🌵 طويل رفيع",                      0.4,  9,     0.4,  0.5 },
    {"💪 ضخم كل شيء (Giant)",             3,    3,     3,    2.5 },
    {"🐜 صغير جداً (Tiny)",               0.3,  0.3,   0.3,  0.3 },
    {"📏 فطيرة (عريض + ممسوط)",           7,    0.2,   7,    0.4 },
    {"🎯 طبيعي (إعادة تعيين)",            1,    1,     1,    1.0 },
}

for _, preset in ipairs(stretchPresets) do
    local pName           = preset[1]
    local px, py, pz, ph = preset[2], preset[3], preset[4], preset[5]
    MakeButton("📏 " .. pName, Color3.fromRGB(28, 50, 105), function()
        scaleX_val = px
        scaleY_val = py
        scaleZ_val = pz
        ApplyBodyScale(px, py, pz, ph)
    end)
end

MakeButton("🔄 إعادة الجسم الأصلي", Color3.fromRGB(80, 28, 28), function()
    ResetBodyScale()
end)

-- حافظ على الـ scale بعد الريسبون (في قسم CharacterAdded لاحقاً)

-- ==================== INFO ====================

MakeSection("ℹ️ Info")
MakeInfoBox("⚡ KSA3396 Hub v4.0\nDeveloper by KSA3396 | Delta Executor\nRightShift = فتح / إغلاق اللوحة\n📏 مط الجسم: يشوفه الكل!\nاضغط على الماب لوضع المعكب المظلم")
...
LocalPlayer.CharacterAdded:Connect(function(char)
    ...
    -- حافظ على مط الجسم بعد الريسبون
    if isScaled then
        task.wait(1.5)
        if not IsR15(char) then SaveR6Originals(char) end
        ApplyBodyScale(currentScaleX, currentScaleY, currentScaleZ)
    end
end)

print("✅ KSA3396 Hub v4.0 Loaded! | Developer by KSA3396")
print("📌 RightShift → فتح / إغلاق | 📏 مط الجسم - يشوفه الكل!")
