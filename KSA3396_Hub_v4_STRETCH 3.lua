...
-- الطريقة: Character:ScaleTo() = دالة رسمية من Roblox، تشتغل R6 وR15، الكل يشوفها
-- + BodyScale NumberValues للـ R15 لتأثير مط غير متساوي

local lastAppliedScale = 1
local scaleSliderVal   = 3

local function DoScale(s)
    local char = LocalPlayer.Character
    if not char then return end
    -- الطريقة الأولى: ScaleTo (الأفضل والأبسط)
    local ok = pcall(function() char:ScaleTo(s) end)
    if not ok then
        -- الطريقة الثانية: R15 BodyScale مباشرة (fallback)
        local hum = char:FindFirstChildOfClass("Humanoid")
        if hum then
            pcall(function()
                for _, n in ipairs({"BodyDepthScale","BodyHeightScale","BodyWidthScale","HeadScale"}) do
                    local v = hum:FindFirstChild(n)
                    if v then v.Value = s end
                end
            end)
        end
    end
...
local function DoStretch(heightScale, widthScale, headScale)
    local char = LocalPlayer.Character
    if not char then return end
    local hum = char:FindFirstChildOfClass("Humanoid")
    -- R15: غير كل بعد بشكل منفصل
    if hum and hum:FindFirstChild("BodyHeightScale") then
        pcall(function()
            hum:FindFirstChild("BodyHeightScale").Value = heightScale
            hum:FindFirstChild("BodyWidthScale").Value  = widthScale
            hum:FindFirstChild("BodyDepthScale").Value  = widthScale
            hum:FindFirstChild("HeadScale").Value       = headScale
        end)
        lastAppliedScale = heightScale
    else
        -- R6 fallback: ScaleTo بمتوسط الأبعاد
        local avg = (heightScale + widthScale * 2) / 3
        pcall(function() char:ScaleTo(avg) end)
        lastAppliedScale = avg
    end
end

-- سلايدر الحجم الكلي
MakeSlider("📐 الحجم الكلي", 1, 10, 3, function(v)
    scaleSliderVal = v
end)

MakeButton("✅ كبّر جسمي (الكل يشوفني)", Color3.fromRGB(20, 120, 50), function()
    DoScale(scaleSliderVal)
end)

MakeButton("🔄 رجوع للحجم الطبيعي", Color3.fromRGB(90, 25, 25), function()
    DoScale(1)
end)

-- أشكال جاهزة
MakeSection("📐 أشكال جاهزة (اضغط مباشرة ✅)")

-- {اسم, height, width, head}
local presets = {
    {"🏔️ ممطوط طويل (زي الصورة)",  6,   0.8, 0.7},
    {"🌵 طويل رفيع جداً",            9,   0.5, 0.5},
    {"🧱 عريض وقصير",                0.6, 5,   1.5},
    {"💪 ضخم كل شيء (Giant x4)",     4,   4,   3.0},
    {"💪 ضخم كل شيء (Giant x6)",     6,   6,   4.0},
    {"🐜 صغير جداً (Tiny)",          0.3, 0.3, 0.3},
    {"📏 فطيرة مبسوطة",              0.2, 6,   0.5},
    {"🔮 كيوب ضخم",                  4,   4,   2.5},
}

for _, p in ipairs(presets) do
    local name, h, w, head = p[1], p[2], p[3], p[4]
    MakeButton(name, Color3.fromRGB(28, 55, 110), function()
        DoStretch(h, w, head)
    end)
end

-- حافظ على الـ scale بعد الريسبون
...
LocalPlayer.CharacterAdded:Connect(function(char)
    ...
    -- حافظ على الحجم بعد الريسبون
    if lastAppliedScale and lastAppliedScale ~= 1 then
        task.wait(1.5)
        pcall(function() char:ScaleTo(lastAppliedScale) end)
    end
end)
