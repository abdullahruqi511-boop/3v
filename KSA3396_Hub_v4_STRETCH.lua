...
--[[
  طريقة الماب الحقيقية:
  نغير BodyScale Values داخل Humanoid مباشرة
  هذي القيم على السيرفر = الكل يشوفها
  + نغير حجم الأجزاء مباشرة كبديل احتياطي
]]

local scaleOriginals = {}
...
local function GetCharScale()
    local char = LocalPlayer.Character
    if not char then return end
    local hum = char:FindFirstChildOfClass("Humanoid")
    if not hum then return end
    return {
        BodyDepthScale  = hum:FindFirstChild("BodyDepthScale"),
        BodyHeightScale = hum:FindFirstChild("BodyHeightScale"),
        BodyWidthScale  = hum:FindFirstChild("BodyWidthScale"),
        HeadScale       = hum:FindFirstChild("HeadScale"),
    }
end

local function SaveScaleOriginals()
    scaleOriginals = {}
    local s = GetCharScale()
    if not s then return end
    for k, v in pairs(s) do
        if v then scaleOriginals[k] = v.Value end
    end
end

local function ApplyBodyScale(sx, sy, sz, headScale)
    local char = LocalPlayer.Character
    if not char then return end
    local hum = char:FindFirstChildOfClass("Humanoid")
    if not hum then return end

    -- الطريقة 1: BodyScale Values (الأفضل - يرى الكل)
    local depthV  = hum:FindFirstChild("BodyDepthScale")
    local heightV = hum:FindFirstChild("BodyHeightScale")
    local widthV  = hum:FindFirstChild("BodyWidthScale")
    local headV   = hum:FindFirstChild("HeadScale")

    if depthV  then depthV.Value  = sz end
    if heightV then heightV.Value = sy end
    if widthV  then widthV.Value  = sx end
    if headV   then headV.Value   = headScale or ((sx + sy + sz) / 3) end
...
local function ResetBodyScale()
    ...
    if depthV  then depthV.Value  = scaleOriginals["BodyDepthScale"]  or 1 end
    if heightV then heightV.Value = scaleOriginals["BodyHeightScale"] or 1 end
    if widthV  then widthV.Value  = scaleOriginals["BodyWidthScale"]  or 1 end
    if headV   then headV.Value   = scaleOriginals["HeadScale"]       or 1 end
...
-- === سلايدرات الأبعاد ===
MakeSlider("↔️ عرض الجسم (X)", 1, 20, 1, function(v)
    scaleX_val = v
    if isScaled then ApplyBodyScale(scaleX_val, scaleY_val, scaleZ_val) end
end)

MakeSlider("↕️ طول الجسم (Y)", 1, 20, 1, function(v)
    scaleY_val = v
    if isScaled then ApplyBodyScale(scaleX_val, scaleY_val, scaleZ_val) end
end)

MakeSlider("↙️ عمق الجسم (Z)", 1, 20, 1, function(v)
    scaleZ_val = v
    if isScaled then ApplyBodyScale(scaleX_val, scaleY_val, scaleZ_val) end
end)

-- === أزرار تطبيق ===
MakeButton("✅ طبّق المط الآن (الكل يشوفه)", Color3.fromRGB(20, 100, 40), function()
    SaveScaleOriginals()
    ApplyBodyScale(scaleX_val, scaleY_val, scaleZ_val)
end)

-- === بريسيتات جاهزة (يشوفها الكل!) ===
...
local stretchPresets = {
    ...
    {"🧱 عريض جداً",                        8,    1,    8,    1.5},
    {"🔮 كيوب ضخم (مربع)",                  5,    5,    5,    2.0},
    {"🪲 مبسوط (عريض + قصير)",              6,    0.3,  6,    0.5},
    {"🌵 طويل رفيع",                        0.3,  10,   0.3,  0.5},
    {"💪 ضخم كل شيء (Giant)",               4,    4,    4,    3.0},
    {"🐜 صغير جداً (Tiny)",                 0.2,  0.2,  0.2,  0.2},
    {"📏 مطوط أفقي (فطيرة)",                8,    0.2,  8,    0.4},
    {"🎯 طبيعي",                            1,    1,    1,    1.0},
}
...
for _, preset in ipairs(stretchPresets) do
    ...
    MakeButton("📏 " .. pName, Color3.fromRGB(30, 55, 100), function()
        SaveScaleOriginals()
        scaleX_val = px
        scaleY_val = py
        scaleZ_val = pz
...
MakeButton("🔄 إعادة الجسم الأصلي", Color3.fromRGB(70, 30, 30), function()
    ResetBodyScale()
end)
